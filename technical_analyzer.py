...
def technical_analysis(prices_df):
    """
    ...
    """
    prices_df[prices_df.columns] = prices_df[prices_df.columns].astype(float)
    ...
    technical_analysis_report = {
        ...
        "strategy_signals": {
            "trend_following": {
                ...
                "description": "Trend following signals based on EMAs and ADX",
            },
            "mean_reversion": {
                "signal": mean_reversion_signals["signal"],
                "confidence": round(mean_reversion_signals["confidence"] * 100),
                "metrics": normalize_pandas(mean_reversion_signals["metrics"]),
                "description": "Mean reversion signals based on z-score and Bollinger Bands",
            },
            "momentum": {
                "signal": momentum_signals["signal"],
                "confidence": round(momentum_signals["confidence"] * 100),
                "metrics": normalize_pandas(momentum_signals["metrics"]),
                "description": "Momentum signals based on price and volume momentum",
            },
            "volatility": {
                "signal": volatility_signals["signal"],
                "confidence": round(volatility_signals["confidence"] * 100),
                "metrics": normalize_pandas(volatility_signals["metrics"]),
                "description": "Volatility signals based on historical volatility and ATR",
            },
            "statistical_arbitrage": {
                "signal": stat_arb_signals["signal"],
                "confidence": round(stat_arb_signals["confidence"] * 100),
                "metrics": normalize_pandas(stat_arb_signals["metrics"]),
                "description": "Statistical arbitrage signals based on price action analysis",
            },
        },
    }
...
def calculate_momentum_signals(prices_df):
    ...
    mom_126 = returns.rolling(126).sum()
    ...
    momentum_score = (0.4 * mom_21 + 0.3 * mom_63 + 0.3 * mom_126).iloc[-1]
    ...
    return {
        ...
        "confidence": confidence,
        "metrics": {
            "momentum_21": float(mom_21.iloc[-1]),
            "momentum_63": float(mom_63.iloc[-1]),
            "momentum_126": float(mom_126.iloc[-1]),
